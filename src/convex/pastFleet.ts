/**
 * Datos históricos — "Control Inventario de Flota Aeropuerto" (formularios pasados).
 *
 * De cada formulario se extraen SOLO 3 datos por unidad:
 *   - SOAT (vencimiento)                 → soatExpiry
 *   - Revisión técnica (vencimiento)     → revisionExpiry
 *   - Próximo servicio (kilometraje)     → nextServiceKm
 *
 * Regla del usuario: si una unidad se repite en varios formularios, NO se
 * sobrescribe — se conserva la primera aparición de cada campo. La mutación
 * `applyHistoricalData` solo rellena campos vacíos y nunca pisa datos existentes.
 *
 * Fechas: dd-mm-aa del formulario → ISO "aaaa-mm-dd".
 *
 * Notas de transcripción:
 *  - "102" con km 71527 / serv. 80955 / SOAT 21-12-26 → es la 0102 (igual a flota de hoy).
 *  - "1064" con km 63892 / serv. 72915 → es la 1061 (igual a flota de hoy).
 *  - "137" con km 32683 / serv. 36626 → es la 0131 (igual a flota de hoy).
 *  - RT "30-2-27" (inválida) en la 0327 → se tomó "20-2-27" (igual a flota de hoy).
 */
...
export const PAST_FLEET: PastFleetEntry[] = [
  ...
  { unitNumber: "0478", nextServiceKm: 78715, soatExpiry: "2026-08-13" },
  ...
  { unitNumber: "0871", nextServiceKm: 54165, soatExpiry: "2026-10-14" },
  ...
  { unitNumber: "0953", nextServiceKm: 10000, soatExpiry: "2027-04-08" },
  { unitNumber: "0962", nextServiceKm: 75647, soatExpiry: "2026-10-04" },
  { unitNumber: "0963", nextServiceKm: 88137, soatExpiry: "2026-11-06" },
  ...
  { unitNumber: "1064", nextServiceKm: 53970, soatExpiry: "2026-07-11" },
